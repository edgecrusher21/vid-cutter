...
namespace ffmpeg {
  ...
  const std::string NULL_STRING( "_NULL"); //this will be a value I will use to declare a string I want null
  ...
  const std::string ffmpegValues(const std::string& inputFile, const std::string& outputFile,
                                 const char * outputFileType, const std::string& startTime,
                                 const std::string& endTime, std::map<std::string,std::string>& md){
    ...
    return command.append(" ").append(DOUBLEQUOTES).append(outputFile).append(DOUBLEQUOTES);
  } 
}
...
//md.insert(std::pair<std::string, std::string>("title", "self bias resistor"));
// md.insert(std::pair<std::string, std::string>("year", "1995"));
//md.insert(std::pair<std::string, std::string>("album", "demanufacture"));
//std::string command = ffmpeg::ffmpegValues("df.mp4", "output.mp4", "mp3", "10:00", "20:00", md);
// const char * output = (ffmpeg::runFFMpeg(command))?"Running command":"command failed";
